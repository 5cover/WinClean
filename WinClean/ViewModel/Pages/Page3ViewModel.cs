...
public sealed class Page3ViewModel : WizardPageViewModel
{
    public Page3ViewModel(CollectionWrapper<IList<ExecutionInfoViewModel>, ExecutionInfoViewModel> executionInfos)
    {
        ...
        ExecutionInfoListViewModel.SelectionChanged += (s, e) => OnPropertyChanged(nameof(SelectedExecutionInfo));
    }

    public ExecutionInfoViewModel? SelectedExecutionInfo => ExecutionInfoListViewModel.CurrentItem;

    public CollectionWrapper<IList<ExecutionInfoViewModel>, ExecutionInfoViewModel> ExecutionInfos { get; }

    public string FormattedElapsedTime => ExecutionInfos.Source.Sum(s => s.Script.ExecutionTime).Humanize(precision: 3, minUnit: TimeUnit.Second);
    ...
    public ExecutionInfoListViewModel ExecutionInfoListViewModel { get; private set; }
}