...
public sealed partial class MainWindow
{
    ...
    private static bool Handle(string path, InvalidDataException e)
    {
        ...
        using Page page = DialogPageFactory.InvalidScriptData(e, path, new() { Button.Retry, Button.Ignore });
        return Button.Retry.Equals(new Dialog(page).Show());
    }
    ...
    private static bool Handle(string path, Exception e)
    {
        ...
        Logs.ScriptLoadError.FormatWith(path, e).Log(LogLevel.Error);
        using Page fsErrorPage = DialogPageFactory.FSError(e, FSVerb.Access, new FileInfo(path), new() { Button.Retry, Button.Ignore });
    ...
    {
        if (Button.Yes.Equals(new Dialog(DialogPageFactory.DeleteScript()).Show()))
        {
